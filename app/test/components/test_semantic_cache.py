...
def cache():
    ...
    semantic_cache_instance = SemanticCache(similarity_threshold=0.8)
    yield semantic_cache_instance
    semantic_cache_instance.vectorstore.delete_collection()

def test_exact_match(cache: SemanticCache):
    ...
    cache.set_cached_response(question, answer, sources)
    result = cache.get_cached_response(question)

    assert result is not None
    assert result['answer'] == answer

def test_semantic_match(cache: SemanticCache):
    ...
    result = cache.get_cached_response(similar_question)

    assert result is not None
    # We expect high similarity, e.g., > 0.8
    assert result['similarity'] > 0.8

def test_cache_miss(cache: SemanticCache):
    """Test cache miss for unrelated question."""
    different_question = "What is the capital of France?"
    result = cache.get_cached_response(different_question)
